...
def profile():
    ...
    # Calculate exams_completed and average_score from exam_results
    exam_results = list(db['exam_results'].find({'username': username}))
    exams_completed = len(exam_results)
    average_score = 0
    if exams_completed > 0:
        total_percentage = sum(res.get('percentage', 0) for res in exam_results)
        average_score = round(total_percentage / exams_completed, 2)

    # Calculate problems_solved if you track it in another collection
    problems_solved = db['practice_submissions'].count_documents({'username': username, 'status': 'solved'})
...
def testtaker_profile():
    ...
    # Only include exams created by this testtaker
    exams_conducted = list(db_exams.find({'created_by': username}))
    exams_conducted_count = len(exams_conducted)
    students_evaluated = sum(exam.get('participated_students', 0) for exam in exams_conducted)
    average_score = 0
    completed_exams = [exam for exam in exams_conducted if exam.get('status', '').lower() == 'completed']
    if completed_exams:
        total_score = sum(exam.get('average_score', 0) for exam in completed_exams)
        average_score = round(total_score / len(completed_exams), 2)
    ...
    profile_data = {
        ...
        'exams_conducted': exams_conducted_count,
        'students_evaluated': students_evaluated,
        'average_score': average_score
    }
